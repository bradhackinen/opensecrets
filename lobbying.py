...
filesAndHeaders = {
                ...
                'issues':('lob_issue.txt',['si_id','filing_id','issue_id','issue','specific_issue','year']),
                'issues_no_specific':('lob_issue_NoSpecficIssue.txt',['si_id','filing_id','issue_id','issue']),
                'bills':('lob_bills.txt',['bill','si_id','congress','bill_name']),
                'lobbyists':('lob_lobbyist.txt',['filing_id','lobbyist_raw','lobbyist','lobbyist_id','year','official_position','congress_id','former_member']),
                'agencies':('lob_agency.txt',['filing_id','agency_id','agency']),
                'industries':('lob_indus.txt',['client','subsidiary','total','year','catcode']),
                'report_types':('lob_rpt.txt',['type_long','type'])
        }
...
def formatFilingsDF(df):
    ...
    for q,dates in dateRangesByType.items():
...
##    for table in['issues']:
        ...
        saveFile = os.path.join(saveDir,table+'.csv')
